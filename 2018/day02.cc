...
std::unordered_map<char, int> mapify(const std::string s) {
...
int part1() {
  std::string s;
  std::ifstream f("data/day02.input");
...
std::string part2() {
  ...
  for (auto u : list) {
    for (auto v : list) {
      int diff_count = 0;
      int len = u.size() < v.size() ? u.size() : v.size();
  ...
  throw("No solution found for part 2.");
}
