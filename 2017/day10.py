...
def part1():
    L = range(256)
    lengths = [int(v) for v in open('data/day10.input').read().split(',')]
...
def part2():
    L = range(256)
    lengths = asciify(open('data/day10.input').read().strip()) + [17, 31, 73, 47, 23]
    pos, skip = 0, 0
    ...
    M = [
        reduce(lambda x,y: x ^ y, L[16 * i:16 * i + 16])
