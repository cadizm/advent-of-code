...
def inverse_captcha1(s):
    return sum([int(a) for a, b in zip(s, s[1:] + s[0]) if a == b])


def part1():
    with open('data/day01.input') as f:
        return inverse_captcha1(f.read().strip())
...
def inverse_captcha2(s):
    ss = [s[(i + len(s) / 2) % len(s)] for i in range(len(s))]
    return sum([int(a) for a, b in zip(s, ss) if a == b])


def part2():
    with open('data/day01.input') as f:
        return inverse_captcha2(f.read().strip())
