...
def part1():
    with open('data/day08.input') as f:
        reg = defaultdict(int)

        for line in f:
            m1 = re.match(r'^(?P<reg>\w+)\s+(?P<op>inc|dec)\s+(?P<arg>\-?\d+)\s+(?P<cond>.*?)$', line.strip())
            m2 = re.match(r'if\s+(?P<cond_reg>\w+)\s+(?P<rel>.*?)$', m1.group('cond'))

            if eval("%d %s" % (reg[m2.group('cond_reg')], m2.group('rel'))):
                reg[m1.group('reg')] = op_map[m1.group('op')](reg[m1.group('reg')], int(m1.group('arg')))
